...
function handleMessage(msg) {
  ...
  if (msg.type === 'GET_FUEL_ESTIMATE') {
    // POST through the same-origin game-tab bridge used by authenticated reads.
...
async function startResearch(researchId, planetId, useFragments = false) {
  ...
  try {
    ...
    const r = await requestFromGameTab(`/api/research/${researchId}/start`, { method: 'POST', body, server });
    if (!r?.ok) return { error: `启动失败：${r?.error || r?.status || '未知错误'}` };
...
async function apiFetch(path, server = null) {
  server ||= await gameServer();
  ...
  for (let attempt = 0; ; attempt++) {
    ...
    let r;
    try {
      r = await requestFromGameTab(path, { server });
    } catch (e) {
    ...
    if (!r.ok) throw new Error(`API ${path} → ${r.status || r.error || '请求失败'}`);
    return r.data || {};
  }
}
...
async function getShipDefs() {
  ...
  try {
    ...
    const [data, me] = await Promise.all([
      apiFetch(`/api/planets/${planetId}/shipyard`, server),
      apiFetch('/api/auth/me', server).catch(() => ({})),
    ]);
...
// global RateLimit-* budget apiFetch tracks, so gate it client-side too.
...
async function gamePost(path, body) {
  ...
  try {
    ...
    for (let attempt = 0; ; attempt++) {
      const r = await requestFromGameTab(path, { method: 'POST', body, server });
...
async function refetchEndpoint(path, server) {
  let json;
  try {
    json = await apiFetch(path, server);
  } catch {
    return;
  }
